...
function upsertResults(classifyRows, auditRows, opts = {}) {
  ...
  for (const r of classifyRows) {
    ...
    let dupFields;
    // `duplicateChecked` records whether the audit ever actually ran for this
    // slug. Without it "audited, none found" and "never looked" are both
    // hasDuplicate:false, and a report can't tell them apart — it would show a
    // confident "No" for a check that never happened.
    if (auditComputed) {
      ...
      dupFields = {
        duplicateChecked: Boolean(prev.duplicateChecked),
